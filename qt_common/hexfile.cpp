...
bool HexFile::append_hex_file(QString file_name) {
  ...
  while (ihex.getData(&hexByte)) {
    uint32_t newAddress =
        ihex.currentAddress(); // fill "blank address spaces" with 0
...
void HexFile::overwrite_binary(const QByteArray &indata,
                               uint32_t start_address) {
  append_binary_(indata, start_address, true);
}

void HexFile::append_binary_(const QByteArray &indata, uint32_t start_address,
                             bool overwrite) {
  if (binary_m.size() == 0) {
    binary_m = indata;
    ...
    if (new_start_address > actual_end_address) {
      ...
      start_address_m = start_address;
    } else if ((overwrite) && (new_start_address > actual_start_address) &&
               (new_end_address < actual_end_address)) {
      ...
      QString msg =
          "There is a memory overlap: (start: 0x" +
          QString::number(new_start_address, 16).toUpper() + ", end: 0x" +
          QString::number(new_end_address, 16).toUpper() +
          ") is added to (start: 0x" +
          QString::number(actual_start_address, 16).toUpper() + ", end: 0x" +
          QString::number(actual_end_address, 16).toUpper() + ")";
      qDebug() << msg;
...
QString HexFile::get_checksum(QString line) {
  ...
  for (int i = 1; i < line.size(); i += 2) { // starting by one to skip the ":"
    ...
    uint8_t byte = byte_str.toUInt(nullptr, 16);
    sum += byte;
  }
...
QStringList HexFile::dump_data_line(uint8_t byte_buffer[MAXHEXLINE],
  ...
  QString dataline =
      QString::asprintf(":%02X%04X", buffer_length, address_lower) + TYPE_DATA;
...
QString HexFile::get_extended_linear_address(uint32_t buffer_addr) {
  ...
  result = ":020000" + TYPE_LINEAR_ADDRESS +
           QString::asprintf("%04X", buffer_addr >> 16);
...
QStringList HexFile::get_hex_file() {
  ...
  for (int i = 0; i < binary_m.size(); i++) {
    ...
    if ((buffer_pos >= MAXHEXLINE)) {
      ...
      buffer_pos = 0;
      result << dump_data_line(byte_buffer, buffer_length, buffer_addr_old,
                               buffer_addr);
  ...
  if (buffer_pos > 0) {
    result << dump_data_line(byte_buffer, buffer_pos, buffer_addr_old,
                             buffer_addr);
  }
...
QByteArray HexFile::get_binary() { return binary_m; }

uint32_t HexFile::get_start_address() { return start_address_m; }
