...
bool FirmwareMetaData::load_meta_data_from_source(ImageCreatorSettings &settings) {
    ...
    while (!symfile_stream.atEnd()) {
        ...
        if (line.contains("firmwareDescriptorBuffer")) {
            ...
            auto fields = line.split(QRegularExpression("[ \\t]"), QString::SkipEmptyParts);
            auto address_firmware_descriptor_in_bootloader_str = "0x" + fields[0];
            bool ok = false;
            address_firmware_descriptor_in_bootloader = address_firmware_descriptor_in_bootloader_str.toInt(&ok, 0);
    ...
    qDebug() << "address_firmware_descriptor_in_bootloader"
             << "0x" + QString::number(address_firmware_descriptor_in_bootloader, 16);
...
QByteArray FirmwareMetaData::compile_firmware_descriptor_binary(const QByteArray &sha256) const {
    QByteArray result;
    ...
    for (int i = 0; i < buf_size; i++) {
        firmware_meta.d.firmwareDescriptor.version[i] = firmware_version[i].toLatin1();
    }
    result.append((char *)&firmware_meta, sizeof(firmware_meta));

    // result[0x2c] = 1;
    result = result.leftJustified(0x50, 0);
    uint16_t crc16 = crc16_buffer(result.data() + 4, sizeof(stm32_firmware_meta_data_t));
    result[0] = crc16 & 0xFF;
