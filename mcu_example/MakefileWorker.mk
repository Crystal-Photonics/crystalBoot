...
#COMPTEST = $(COMPILE.cpp)
#COMPILE.cpp = C:/MinGW32-xy/bin/g++
...
time = $(shell C:\Programme_local\git_bin\bin\date.exe +%s)
...
GCOV_CLEAN = $(SILENCE)$(UNIXTOOLS)rm -f $(GCOV_GCDA_FILES) $(GCOV_OUTPUT) $(GCOV_REPORT) $(GCOV_ERROR)
#RUN_TEST_TARGET = $(SILENCE)  $(GCOV_CLEAN) ; echo "Running $(TEST_TARGET)"; ./$(TEST_TARGET).exe $(CPPUTEST_EXE_FLAGS)
RUN_TEST_TARGET = $(TEST_TARGET).exe $(CPPUTEST_EXE_FLAGS)
...
$(TARGET_LIB): $(OBJ)
	...
	$(UNIXTOOLS)mkdir -p $(dir $@)
	$(AR) $(ARFLAGS) $@ $^
	$(RANLIB) $@

#	$(SILENCE)	$(UNIXTOOLS)mkdir -p $(dir $@)
#	$(SILENCE)$(AR) $(ARFLAGS) $@ $^
#	$(SILENCE)$(RANLIB) $@
...
clean:
	@echo Making clean
	$(SILENCE)$(UNIXTOOLS)rm $(STUFF_TO_CLEAN)
	$(SILENCE)$(UNIXTOOLS)rm -rf gcov $(CPPUTEST_OBJS_DIR)
	$(SILENCE)$(UNIXTOOLS)find . -name "*.gcno" | xargs $(UNIXTOOLS)rm -f
	$(SILENCE)$(UNIXTOOLS)find . -name "*.gcda" | xargs $(UNIXTOOLS)rm -f

#realclean gets rid of all gcov, o and d files in the directory tree
#not just the ones made by this makefile
.PHONY: realclean
realclean: clean
	$(SILENCE)$(UNIXTOOLS)rm -rf gcov
	$(SILENCE)$(UNIXTOOLS)find . -name "*.gdcno" | xargs $(UNIXTOOLS)rm -f
	$(SILENCE)$(UNIXTOOLS)find . -name "*.[do]" | xargs $(UNIXTOOLS)rm -f
...
ifeq ($(CPPUTEST_USE_VPATH), Y)
	...
	$(SILENCE)$(UNIXTOOLS)cat $(GCOV_REPORT)
	$(SILENCE)$(UNIXTOOLS)mkdir -p gcov
	$(SILENCE)$(UNIXTOOLS)mv *.gcov gcov
	$(SILENCE)"$(UNIXTOOLS)mv gcov_* gcov
