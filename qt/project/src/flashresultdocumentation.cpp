...
void FlashResultDocumentation::save(CrystalBootSettings crystalBootSettings) {
    ...
    const FirmwareMetaData &fw_meta_data = firmwareImage.get_firmware_meta_data();

    npm.addVariable("%IMAGE_FILE_PATH%", QFileInfo(firmwareImage.fileName).absoluteDir().absolutePath());
    npm.addVariable("%FIRMWARE_GIT_HASH%", fw_meta_data.getGitHash_str());
    npm.addVariable("%FIRMWARE_NAME%", fw_meta_data.firmware_name);
    npm.addVariable("%FIRMWARE_VERSION%", fw_meta_data.firmware_version);
    ...
    xml.writeAttribute("githash", fw_meta_data.getGitHash_str());
    xml.writeAttribute("gitdate", fw_meta_data.firmware_gitdate_dt.toString("yyyy.MM.dd HH:mm"));
    xml.writeAttribute("gitdate_unix", QString::number(fw_meta_data.firmware_githash));
    xml.writeAttribute("version", fw_meta_data.firmware_version);
    xml.writeAttribute("name", fw_meta_data.firmware_name);
    xml.writeAttribute("name_crc", "0x" + QString::number(fw_meta_data.getNameCRC16(), 16).toUpper());
    xml.writeAttribute("entryPoint", "0x" + QString::number(fw_meta_data.firmware_entryPoint, 16).toUpper());
    xml.writeAttribute("size_hex", "0x" + QString::number(fw_meta_data.firmware_size, 16).toUpper());
    xml.writeAttribute("size_dec", QString::number(fw_meta_data.firmware_size));

    if (fw_meta_data.crypto == FirmwareMetaData::Crypto::Plain) {
        xml.writeAttribute("crypto", "plain");
    } else if (fw_meta_data.crypto == FirmwareMetaData::Crypto::AES128) {
...
void FirmwareUpdatePlausibilityCheck::checkPlausibiltity(RemoteDeviceInfo remoteDevInfo, FirmwareImage firmwareImage) {
    ...
    const FirmwareMetaData &fw_meta_data = firmwareImage.get_firmware_meta_data();
    uint32_t fw_lastAddress = fw_meta_data.firmware_entryPoint + fw_meta_data.firmware_size;
    ...
    if ((fw_meta_data.firmware_entryPoint != remoteDevInfo.mcu_descriptor.firmwareEntryPoint) &&
        (remoteDevInfo.mcu_descriptor.firmwareEntryPoint != 0xFFFFFFFF)) {
        plausibilityResults.insert(PlausibilityResult::error_wrong_entrypoint);
    }

    if (fw_meta_data.firmware_entryPoint < remoteDevInfo.mcu_descriptor.minimalFirmwareEntryPoint) {
        plausibilityResults.insert(PlausibilityResult::error_wrong_entrypoint);
    }

    if ((fw_meta_data.crypto == FirmwareMetaData::Crypto::Plain) && (remoteDevInfo.mcu_descriptor.cryptoRequired)) {
    ...
    if (fw_meta_data.getNameCRC16() != remoteDevInfo.firmwareDescriptor.nameCRC16) {
        plausibilityResults.insert(PlausibilityResult::warning_different_name_hash);
    }

    if (fw_meta_data.firmware_gitdate_unix < remoteDevInfo.firmwareDescriptor.gitDate_unix) {
        plausibilityResults.insert(PlausibilityResult::warning_downgrade_date);
    }

    if (fw_meta_data.firmware_githash == remoteDevInfo.firmwareDescriptor.githash) {
...
QString FirmwareUpdatePlausibilityCheck::plausibilityResultToStrReadable(PlausibilityResult plauRes) {
    switch (plauRes) {
        ...
        case PlausibilityResult::warning_different_name_hash:
            return QString(
                "warning: different name hashes. Installed Firmware has different name than this one. You are installing a different software.");
