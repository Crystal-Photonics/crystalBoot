...
void MainWindow::loadFirmwarePathUIFromFile() {
    ...
    // QString elidedPath = fontMetrics.elidedText("bootloader.fileNameToSend sdgfdsfgdfg dsfgdsfgdfg dsfgdfgdfgdf/dgfsdgfdfg dsfgdsfg",
    // Qt::ElideLeft, ui->lbl_nf_path->width());
...
void MainWindow::loadUIDeviceInfo() {
    if (bootloader.remoteDeviceInfo.isValid()) {
        ...
        ui->plainTextEdit->appendPlainText(QString("MCU devID: ") + bootloader.remoteDeviceInfo.getMCU_DeviceIDString() + " " +
                                           bootloader.remoteDeviceInfo.getDeviceCategorieString());
        ui->plainTextEdit->appendPlainText(QString("MCU revID: ") + bootloader.remoteDeviceInfo.getMCU_RevString());
        ui->plainTextEdit->appendPlainText(QString("Flashsize: ") + QString::number(bootloader.remoteDeviceInfo.mcu_descriptor.flashsize / 1024) +
                                           "kB (" + bootloader.remoteDeviceInfo.getMCU_FlashsizeString() + ")");
    ...
    if (bootloader.remoteDeviceInfo.isValid() && bootloader.fwImage.isValid()) {
...

        {
            ...
            if (plauResult.count(PlausibilityResult::warning_equal_version_but_different_gitHash)) {
                ...
                if ((plauResult.count(PlausibilityResult::warning_equal_gitHash) == 0) &&
                    (plauResult.count(PlausibilityResult::warning_equal_version_but_different_gitHash) == 0)) {
                    ui->lbl_nf_githash->setPalette(palette_ok);
                    ui->lbl_rf_githash->setPalette(palette_ok);
                }
                if ((plauResult.count(PlausibilityResult::warning_downgrade_date) == 0) &&
                    (plauResult.count(PlausibilityResult::warning_equal_gitHash_but_different_wersion) == 0)) {
                    ui->lbl_nf_version->setPalette(palette_ok);
                }
            }
        }
...
void MainWindow::refreshComPortList() {
    cmbPort->clear();
    cmbPort->addItem("");
    foreach (const QSerialPortInfo &info, QSerialPortInfo::availablePorts()) { cmbPort->addItem(info.portName()); }
    cmbPort->addItem("");
